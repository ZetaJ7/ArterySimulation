...
def substep():
    ...
    for i, j in grid_m:
        if grid_m[i, j] > 0:  # No need for epsilon here
            ...
            dist2 = attractor_pos2[0] - dx * ti.Vector([i, j])
            dist3 = attractor_pos2[1] - dx * ti.Vector([i, j])
            dist4 = attractor_pos2[2] - dx * ti.Vector([i, j])
            grid_v[i, j] += dist / (0.01 + dist.norm()) * attractor_strength[None] * dt * 100 + dist2 / (0.01 + dist2.norm()) * attractor_strength2[None] * dt * 100 + dist3 / (0.01 + dist3.norm()) * attractor_strength2[None] * dt * 100 + dist4 / (0.01 + dist4.norm()) * attractor_strength2[None] * dt * 100
...
for frame in range(20000):
    ...
    # Exit Force
    attractor_strength2[None] = 1
    # attractor_strength2[None] = 2 + 1 * ti.sin(math.pi * 0.03 * frame)

    for s in range(int(2e-3 // dt)):
        substep()
    colors = np.array([0xC0C0C0, 0x000000, 0x00008b], dtype=np.uint32)
